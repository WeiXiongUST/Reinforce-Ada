...
def is_correct_minerva(
    ...
    # 【修改点 1】: 不再使用正则表达式，而是调用 last_boxed_only_string
    boxed_answer = last_boxed_only_string(solution_str)
    # 【修改点 2】: 如果没有找到 boxed 答案，就标记为无效
    if boxed_answer is None:
        extracted_answer = "[INVALID]"
    else:
        extracted_answer = boxed_answer # 注意：此时还带着 \boxed{}

    pred = normalize_final_answer(extracted_answer)

    # Process ground truth
    if gt_need_extract:
        # 如果标准答案也需要提取，我们也用同样的方式处理它
        gt_boxed = last_boxed_only_string(gt)
        gt = normalize_final_answer(remove_boxed(gt_boxed)) if gt_boxed else normalize_final_answer(gt)
    else:
